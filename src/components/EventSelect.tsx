...
import { IconChevronDown, IconSwimmer } from './icons'
...
export function EventSelect({ value, onChange, disabled }: EventSelectProps) {
  ...
  return (
    <section className="event-select-section">
      <label className="field-label" htmlFor="plan-event">
        {t('eventSelect.heading')}
      </label>
      <div className="event-select-wrap">
        <IconSwimmer className="event-select-icon" size={20} />
        <select
          id="plan-event"
        ...
        <IconChevronDown className="event-select-chevron" size={18} />
      </div>
    </section>
  )
}
